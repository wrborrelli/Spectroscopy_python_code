...
def compute_polarized_tdm(dipoles, steps_in_t_delay, q_func, pol_type):
    ...
    if pol_type == "parallel":
        edips = np.array([x_dip, x_dip, x_dip, x_dip])
    elif pol_type == "perpendicular":
        edips = np.array([x_dip, x_dip, y_dip, y_dip])
    elif pol_type == "cross":
        edips = np.array([x_dip, y_dip, x_dip, y_dip])
    else:
